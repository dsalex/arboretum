...
namespace arboretum {
  namespace core {
    ...
    __global__ void gather_kernel_opt(const int *position, const type1 *in1, type1 *out1, const size_t n){
      const unsigned int partitionSize = 1024*256;
      ...
      for (unsigned int i = blockDim.x * blockIdx.x + threadIdx.x;
          ...
          out1[pos] =  pos - partition <= partitionSize ? in1[pos] : out1[pos];
        }
    }
    ...
    public:
      GardenBuilder(const TreeParam &param, const io::DataMatrix* data, const unsigned short overlap) : overlap_depth(overlap),
        ...
        cudaOccupancyMaxPotentialBlockSize( &minGridSize, &blockSizeGather, gather_kernel_opt<float_type>, 0, 0);
        ...
        parent_count_iter.BindTexture(thrust::raw_pointer_cast(parent_node_count.data()), sizeof(int) * (lenght + 1));
                      ...
                      for(size_t fid = 0; fid < data->columns; ++fid){
                          ...
                          for(size_t i = 0; i < overlap_depth && (fid + i) < data->columns; ++i){
                              ...
                              gather_kernel_opt<<<gridSizeGather, blockSizeGather, 0, s >>>(thrust::raw_pointer_cast(index_tmp->data()),
